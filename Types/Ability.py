...
class List(object):
    ...
    def __init__(self, abilities = [], allIs = None):
        ...
        """
        self.__unicode__ = self.__obj_unicode__
        ...
        def canonize(simple):
            ...
            if not Grimoire.Utils.isSubclass(policy, Category):
                policy = (Deny, Allow)[not not policy]
    ...
    # Emulate a Python set containing all allowed methods
    ...
    def issubset(self, other):
        ...
        return self.difference(other).partition()[0].abilities == []
        
    def issuperset(self, other):
        """Self is a superset of other"""
        return other.difference(self).partition()[0].abilities == []

    # Treats the list as two separate sets - the sets of all denied
    # methods and the set of all allowed methods.
    ...
    # Methods for changing the "view" of the list. These are used by
    # the methods above.
    ...
    # Treat a list that contains either only allows or only denys, as
    # the allowed or denied set, respectively.
    ...
    # Ugglyhack together with the line in __init__ to be able to do unicode(List) and unicode(List())
    def __unicode__(cls):
        return object.__str__(cls)
    __unicode__ = classmethod(__unicode__)
    
    def __obj_unicode__(self):
        return u'Ability:\n ' + u'\n '.join(Grimoire.Utils.Map(lambda x: unicode(x[0].__name__) + u': ' + unicode(x[1]),
                                                         self.abilities)) + u'\n'
